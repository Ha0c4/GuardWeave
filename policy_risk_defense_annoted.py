...
import hashlib  # 哈希：用于生成稳定的规则ID（避免把regex明文写进可见输出/日志）
...
def _short_hash(text: str, n: int = 12) -> str:
    ...
    用途：
    - 生成 deny_pattern 的稳定ID，避免把 regex 明文写进用户可见输出或公开日志。

    param text: input string.
    param n: length of the short id.
    return: short hex id.
    """
    if text is None:
        text = ""
...
class GeminiRiskJudge:
    """使用Gemini REST API的LLM作为裁判的风险评分器 范围是0-1。
...
class GeminiOutputJudge:
    """使用Gemini REST API的LLM作为裁判的输出合规性检查器 检查output是否违反策略，返回（是否违反，原因）。
...
class DefenderConfig:
    ...
    # 是否在用户可见拒绝文案中暴露内部原因。
    # 论文/攻防评估场景：默认必须为 False，避免把 deny_pattern/regex 等防御指纹当作 side-channel 吐给攻击者。
...
class PolicyRiskDefender:
    ...
    def after_generate(self, user_text: str, model_output: str, controls: Controls) -> Dict[str, Any]:
        ...
        decision_path: List[str] = []  # 记录决策链路（用于审计/论文复现）
        overrides: Dict[str, Any] = {}  # 记录 override/证据（不要写 regex 明文）
    ...
    def _trend_suspicion(self, *, user_text: str, risk: float, chunk_sig: bool) -> float:
        ...
        suspicion += 0.50 if slope_hit else 0.0  # 如果斜率命中，加0.50
        suspicion += 0.30 if probe_hit else 0.0  # 如果探测命中，加0.30
        suspicion += 0.20 if chunk_hit else 0.0  # 如果分块命中，加0.20
    ...
    def _heuristic_output_violation(self, output_text: str) -> Tuple[bool, str]:
        ...
        for dp in self.policy.deny_patterns:  # 遍历拒绝模式
            if dp.search(output_text):  # 如果匹配拒绝模式
                # 注意：不要把正则明文当作 reason 传播到用户可见输出/可公开日志。
                # 这里仅返回稳定ID，便于审计与复现（同一规则恒定）。
                pid = _short_hash(dp.pattern)
                return True, f"deny_pattern_id:{pid}"  # 返回违规和原因
    ...
    def _refusal_text(self, reason: str) -> str:
        ...
        # 默认：不向用户暴露内部原因，避免把 deny_pattern/regex/judge 细节当作 side-channel。
        if not getattr(self.cfg, "expose_refusal_reason_to_user", False):
            return base

        # 即使显式允许，也只输出抽象化 reason code，避免泄露规则明文。
